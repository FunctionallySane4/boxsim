...
task runr, "Runs boxsim for current platform":
 exec &"nim c -r {releaseOpts} -o:boxsim src/main.nim"

task rund, "Runs debug boxsim for current platform":
 exec &"nim c -r {debugOpts} -o:boxsim src/main.nim"

task release, "Builds boxsim for current platform":
 exec &"nim c {releaseOpts} -o:boxsim src/main.nim"

task webd, "Builds debug boxsim for web":
 exec &"nim c {debugOpts} -d:emscripten -o:boxsim.html src/main.nim"

task webr, "Builds release boxsim for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:boxsim.html src/main.nim"

task debug, "Builds debug boxsim for current platform":
 exec &"nim c {debugOpts} -o:boxsim_debug src/main.nim"
